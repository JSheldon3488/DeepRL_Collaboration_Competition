...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        states = torch.from_numpy(np.asarray([e.states for e in experiences if e is not None])).float().to(device)
        actions = torch.from_numpy(np.asarray([e.actions for e in experiences if e is not None])).float().to(device)
        rewards = torch.from_numpy(np.vstack([e.rewards for e in experiences if e is not None])).float().to(device)
        next_states = torch.from_numpy(np.asarray([e.next_states for e in experiences if e is not None])).float().to(device)
